...
//        groceriesTableView.reloadData()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        ItemLabel.text = groceries[indexPath.row]["Item"]!
        ...
        dueDateLabelOnCell.leftAnchor.constraint(equalTo: DescLabelOnCell.rightAnchor, constant: 2.5).isActive = true
        
        DescLabelOnCell.topAnchor.constraint(equalTo: (dueDateLabelOnCell.bottomAnchor), constant: 5).isActive = true
        ItemLabel.rightAnchor.constraint(equalTo: cell.rightAnchor, constant: -2.5).isActive = true
        ItemLabel.leftAnchor.constraint(equalTo: CreatorLabelOnCell.rightAnchor, constant: 2.5).isActive = true
        
        DescLabelOnCell.topAnchor.constraint(equalTo: (cell.topAnchor), constant: 5).isActive = true
        DescLabelOnCell.leftAnchor.constraint(equalTo: cell.leftAnchor, constant: 2.5).isActive = true
        DescLabelOnCell.rightAnchor.constraint(equalTo: dueDateLabelOnCell.leftAnchor, constant: -2.5)
        
        CreatorLabelOnCell.topAnchor.constraint(equalTo: (DescLabelOnCell.bottomAnchor), constant: 5).isActive = true
        CreatorLabelOnCell.leftAnchor.constraint(equalTo: cell.leftAnchor, constant: 2.5).isActive = true
        CreatorLabelOnCell.rightAnchor.constraint(equalTo: DescLabelOnCell.leftAnchor, constant: -2.5).isActive = true
...
//    }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath)
    {
        if (editingStyle == UITableViewCellEditingStyle.delete) {
            ...
            if groceries[indexPath.row]["creator"] == userName {
            ...
            if let values = snap.value as? NSDictionary {
                for key in values.allKeys{
                    ...
                    if self.tabbar.selectedItem! == self.TodoItem && self.dateFormatter.date(from: value["due_on"] as! String)!.compare(Date()) == .orderedDescending{
                        let dict = ["title": value["title"],"desc": value["description"],"creator":"Created by: \(value["creator"]!)","dueDate":"Due on: \(value["due_on"]!)", "id": "\(key)"]
